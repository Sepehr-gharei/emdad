...
function fix_post_date($time, $format = ''){
  global $post;
  ...
  $time = jdate($format, $timestamp);

  return $time;
}

function fix_post_time($time, $format = ''){
  global $post;
...
function fix_comment_time($time, $format = ''){
  global $comment;
...
function fix_comment_date($time, $format = ''){
  global $comment;
...
function fix_i18n($date, $format, $timestamp, $gmt) {
  global $post;
...
function mdate_to_jdate($date) {
    $timestamp = strtotime($date);
    $jalaliDate = jdate('Y-m-d H:i:s', $timestamp);
    return $jalaliDate;
}

add_filter('the_time', 'fix_post_time', 10, 2);
add_filter('the_date', 'fix_post_date', 10, 2);
add_filter('get_the_time', 'fix_post_date', 10, 2);
add_filter('get_the_date', 'fix_post_date', 100, 2);
add_filter('get_comment_time', 'fix_comment_time', 10, 2);
add_filter('get_comment_date', 'fix_comment_date', 10, 2);
add_filter('date_i18n', 'fix_i18n', 10, 4);
add_filter('wp_date', 'fix_i18n', 10, 4);